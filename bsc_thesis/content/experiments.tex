...
A base image is created as explained above. Ten further images are created by scaling the image with factors from 0.5 to 1.5 of the original size. The objects are triangles, rectangles and ellipses. Samples can be seen in \ref{fig:scale_samples}.
...
To reproduce the results of \cite{ScalingTheScatteringTransform2017} regarding classification a network is trained on a toy dataset created for classification. Every image contains exactly one triangle, ellipse or rectangle in different sizes and colors. Samples of the images are shown in figure \ref{fig:toy_data_class_samples}.
