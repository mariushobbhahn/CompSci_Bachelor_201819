"""
Self-contained classification example on ToyData
...
#toy_data_path = os.path.abspath('/home/marius/data/toy_data_classificaton')
if module_path not in sys.path:
    sys.path.append(module_path)
sys.path.append(scattering_path)
#sys.path.append(toy_data_path)
...
from toy_data_loader import ToyData
import numpy as np
...
class Scattering2dResNet(nn.Module):
    ...
    def forward(self, x):
        print("x.size(): ",x.size())
        x = x.view(x.size(0), self.K, 32, 32) #changed from 8 by 8 because the image is 128 instead of 32
        x = self.init_conv(x)
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Toy_data scattering  + hybrid examples')
    ...
    if args.mode == 1:
        scattering = Scattering2D(M=128, N=128, J=2,order2=False)
        K = 17*3
    else:
        scattering = Scattering2D(M=128, N=128, J=2)
        K = 81*3
    if use_cuda:
    ...
    load_file = '/home/marius/data/toy_data_classification/toy_data_classificaton.npy'
    train_images, train_labels, test_images, test_labels = np.load(load_file)
    print("train_images size: ", np.shape(train_images))
    print("train_labels size: ", np.shape(train_labels))
    print("test_images size: ", np.shape(test_images))
    print("test_labels size: ", np.shape(test_labels))

    train_dataset = ToyData(images=train_images, labels=train_labels)
    test_dataset = ToyData(images=test_images, labels=test_labels)
