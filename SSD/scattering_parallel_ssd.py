...
class Scattering2dParallelSSD(nn.Module):
    ...
    def forward(self, x, scatterings, Ks):
        ...
        if self.batch_norm:
            ...
            x = self.vgg[14](x) #256
            x = self.vgg[15](x) #bn
            x = self.vgg[16](x) #relu
            x = self.vgg[17](x) #256
            x = self.vgg[18](x) #bn
            x = self.vgg[19](x) #relu
            x = self.vgg[20](x) #256
            x = self.vgg[21](x) #bn
            ...
            x = self.vgg[24](x) #512
            x = self.vgg[25](x) #bn
            x = self.vgg[26](x) #ReLU
            x = self.vgg[27](x) #512
            x = self.vgg[28](x) #bn
            x = self.vgg[29](x) #ReLU
            x = self.vgg[30](x) #512
            x = self.vgg[31](x) #bn
            x = self.vgg[32](x) #ReLU
        else:
            x = self.vgg[0](x) #64
            x = self.vgg[1](x) #relu
            x = self.vgg[2](x) #64
...
def vgg(cfg, i, batch_norm=False):
    ...
    if batch_norm:
        ...
        layers += [conv1_1, bn64, relu, conv1_2, bn64, relu, max_pool]
    else:
    ...
    if batch_norm:
        ...
        layers += [conv2_1, bn128, relu, conv2_2, bn128, relu, max_pool]
    else:
        layers += [conv2_1, relu, conv2_2, relu, max_pool]
    #scattering2 is added here, therefore the 371 = 128 + 243
    conv3_1 = nn.Conv2d(371, 256, kernel_size=3, padding=1)
    ...
    if batch_norm:
        ...
        layers += [conv3_1, bn256, relu, conv3_2, bn256, relu, conv3_3, bn256, relu, max_pool_ceil]
    else:
    ...
    if batch_norm:
        bn512 = nn.BatchNorm2d(512)
        layers += [conv4_1, bn512, relu, conv4_2, bn512, relu, conv4_3, bn512, relu, max_pool]
    else:
    ...
    if batch_norm:
        layers += [conv5_1, bn512, relu, conv5_2, bn512, relu, conv5_3, bn512, relu]
    else:
        layers += [conv5_1, relu, conv5_2, relu, conv5_3, relu]
    
    #no batch norm for the last layers
...
def multibox(vgg, extra_layers, cfg, num_classes, batch_norm):
    loc_layers = []
    conf_layers = []
    vgg_source = [21 + 9 if batch_norm else 21, -2]       #corresponds to conv4_3 and conv7
...
def build_scattering_parallel_ssd(phase, inp_channels, size_x=300, size_y=300, num_classes=21, cfg='voc', pretrained=False, batch_norm=False):
    ...
    return Scattering2dParallelSSD(phase=phase, inp_channels=inp_channels,
                           ...
                           head=head_, num_classes=num_classes, cfg=cfg, batch_norm=batch_norm, pretrained_weights=pretrained)
