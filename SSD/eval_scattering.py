...
parser.add_argument('--trained_model',
                    default='weights/scattering_ssd_J2_toy_data_pretrained_random_12.8_125000.pth', type=str, help='Trained state_dict file path to open')
...
parser.add_argument('--J', default=2, type=int, help='size of the scattering window per filter')
parser.add_argument('--pretrained_weights', default=True, help='use pretrained weights for VGG')
...
if args.dataset == 'VOC':
    ...
    imgpath = os.path.join(dataset_root, 'VOC2007', 'JPEGImages', '%s.jpg')
    imgsetpath = os.path.join(dataset_root, 'VOC2007', 'ImageSets',
                          'Main', '{:s}.txt')
    YEAR = '2007'
    devkit_path = dataset_root + 'VOC' + YEAR
...
def do_python_eval(output_dir='output', use_07=True):
    ...
    for i, cls in enumerate(labelmap):
        ...
        #print("output_dir during do_python_eval: ", output_dir)
...
# first load gt
    if not os.path.isdir(cachedir):
        os.mkdir(cachedir)
    cachefile = os.path.join(cachedir, 'annots_{}.pkl'.format(set_type))
    # read list of images
...
def test_net(save_folder, cuda, dataset, transform, top_k, scattering, inp_channels,
             im_size=300, thresh=0.0):

    net = build_scattering_ssd(phase='test', inp_channels=inp_channels, size_x=cfg['dim_x'], size_y=cfg['dim_y'], num_classes=cfg['num_classes'], cfg=cfg, pretrained=args.pretrained_weights)
    ...
    all_boxes = [[[] for _ in range(num_images)]
                 for _ in range(len(labelmap) + 2)]

    # timers
    _t = {'im_detect': Timer(), 'misc': Timer()}
    if args.dataset == "VOC":
        output_dir = get_output_dir('ssd300_120000', set_type)
    elif args.dataset == 'kitti_voc' or 'kitti_voc_small':
        output_dir = get_output_dir('ssd300_120000_kitti', set_type)
    elif args.dataset == 'toy_data':
        output_dir = get_output_dir('ssd300_120000_toy_voc', set_type)
    ...
    for i in range(num_images):
        ...
        if args.cuda:
            x = x.cuda()
        _t['im_detect'].tic()
        x = scattering(x) #add scattering transform
...
if __name__ == '__main__':
    ...
    # evaluation
    test_net(save_folder=args.save_folder, cuda=args.cuda, dataset=dataset, scattering=scattering,
             transform=BaseTransform(cfg['dim_x'], cfg['dim_y'], dataset_mean, normalize=args.normalize), top_k=args.top_k, im_size=cfg['dim_y'],
             inp_channels=K, thresh=args.confidence_threshold)
